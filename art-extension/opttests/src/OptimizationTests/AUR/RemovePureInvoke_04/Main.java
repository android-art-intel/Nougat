...
package OptimizationTests.AUR.RemovePureInvoke_04;
...
public class Main {
    ...
    private int foo(String s, int arg ) {
        return s.lastIndexOf(arg);
    }
    ...
    // pure non-static non-intrinsic subsequent calls + inlining 
    public double runTest (int x, int n, Main m, Main other, String a, int b, int c) {
      ...
      b = foo(a, foo(a, foo(a, foo(a, 1)))); // foo is inlined with pure non-static non-intrinsic invoke of lastIndexOf; NullCheck and InvokeStaticOrDirect have parent environemnt from foo;
      // NullCheck for object m will containt InvokeStaticOrDirect in its env
      m.field++;
        if (n > 6) {
            c += b;
        }
    ...
    public int test(int n) {
        Main m = new Main();
        Main other = new Main();
        return (int)runTest(11, n, m, other, "45d", 7, -10);
    }
