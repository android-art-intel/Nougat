...
package OptimizationTests.OSR.LoopPeeling_SetterGetter_01;
...
public class Main {
    public final int iterations = 100*10000;
    ...
    public double test_07(int x) {
        ...
        for (float i = iterations; i > 0; i -= 1.33f) {
            obj_array[1].arr_field1[0] = -1;
    ...
    public int test_08(int n) {
        ...
        int m = n - 1;
        for (int i = 0; i < c.getArrInlined(m).length*iterations; i++) {
            i++;
            b[i/iterations] = true;
            c1.count = 5;
            testVar = c1.count;
