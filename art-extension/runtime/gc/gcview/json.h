...
#ifndef ART_RUNTIME_GC_GCVIEW_JSON_H_

#define ART_RUNTIME_GC_GCVIEW_JSON_H_
...
private:
  ...
  void writeSeparator(unsigned count, bool add_newline) {
    ...
    if (add_newline) {
      writeNewline();
      unsigned lines = _active_with_newlines - 1;
      for (unsigned i = 0; i < lines; i += 1) {
        baseWrite(" ");
      }
    }
    baseWrite(" ");
...
public:
  JSONWriter(FILE* fout = stdout)
      : _fout(fout), _file_name(nullptr),
        _active_objects(0), _active_arrays(0), _active_with_newlines(0) { }

  JSONWriter(const char* file_name)
      : _fout(nullptr), _file_name(file_name),
        _active_objects(0), _active_arrays(0), _active_with_newlines(0) {
    GCVIEW_ASSERT(file_name != nullptr);
    _fout = fopen(file_name, "w");
    GCVIEW_GUARANTEE(_fout != nullptr, "could not open file");
  }
...
#endif // ART_RUNTIME_GC_GCVIEW_JSON_H_
