...
#ifndef ART_RUNTIME_GC_GCVIEW_GCVIEW_H_

#define ART_RUNTIME_GC_GCVIEW_GCVIEW_H_
...
private:
  ...
  void updateGCviewSpaceData(const char* gc_cause, double collection_time_sec);

public:
  GCviewSpace* addSpace(const char* space_name);
  GCviewSpace* addSpace(GCviewSpace* space);
  GCviewSpace* findSpace(const char* name, bool should_succeeded = true) const;
  ...
  void eventEnd(const char* gc_cause, double now_sec = -1.0);
...
#endif // ART_RUNTIME_GC_GCVIEW_GCVIEW_H_
