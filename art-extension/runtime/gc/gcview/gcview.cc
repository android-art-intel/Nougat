...
#define ITERATE_SPACES(__cmd__) \
  GCVIEW_ARRAY_ITERATE(&_spaces, GCviewSpace*, the_space, __cmd__)
...
void GCview::updateGCviewSpaceData(const char* gc_cause, double collection_time_sec) {
  _gc_cause_value->value() = gc_cause;
...
GCviewSpace* GCview::addSpace(const char* name) {
  GCviewSpace* space = new GCviewSpace(name);
  GCVIEW_ALLOC_GUARANTEE(space);
  return addSpace(space);
}

GCviewSpace* GCview::addSpace(GCviewSpace* space) {
  GCVIEW_GUARANTEE(findSpace(space->getName(), false /* should_succeed */) == nullptr,
...
GCviewSpace* GCview::findSpace(const char* name, bool should_succeed) const {
  ITERATE_SPACES({
...
void GCview::eventEnd(const char* gc_cause, double now_sec) {
  GCVIEW_ASSERT(_last_event_start_timestamp_sec >= 0.0);
  ...
  updateGCviewSpaceData(gc_cause, collection_time_sec);
}
...
GCview::GCview(const char* name, double now_sec)
    : _modified(false), _start_sec(now_sec),
      ...
      _event_value(nullptr), _gc_cause_value(nullptr), _total_event_count_value(nullptr),
      _elapsed_time_value(nullptr), _actual_elapsed_time_value(nullptr),
      _last_data_collection_time_value(nullptr),
      _total_data_collection_time_value(nullptr),
      _event_names_array(nullptr), _event_counts_array(nullptr) {
  updateLastTimestampSec(now_sec);
