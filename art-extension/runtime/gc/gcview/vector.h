...
#ifndef ART_RUNTIME_GC_GCVIEW_VECTOR_H_

#define ART_RUNTIME_GC_GCVIEW_VECTOR_H_
...
public:
  ...
  void resize(unsigned new_length) {
    if (new_length != _length) {
      ...
      if (new_length > 0) {
        GCVIEW_ASSERT(new_array != nullptr);
        _array = new_array;
        _length = new_length;
      }
    }
  ...
  Vector() : _length(0), _array(nullptr) { }
  ~Vector() { reclaim(); }
};
...
#endif // ART_RUNTIME_GC_GCVIEW_VECTOR_H_
