 ...
 protected:
  ...
  void CheckQuickEntryPoints() {
    ...
    EXPECT_OFFSET_DIFFNP(QuickEntryPoints, pReadBarrierSlow, pReadBarrierForRootSlow,
                         sizeof(void*));
    EXPECT_OFFSET_DIFFNP(QuickEntryPoints, pReadBarrierForRootSlow, pJniMethodStartFromCode,
                         sizeof(void*));
    EXPECT_OFFSET_DIFFNP(QuickEntryPoints, pJniMethodStartFromCode,
                         pJniMethodStartSynchronizedFromCode, sizeof(void*));

    EXPECT_OFFSET_DIFFNP(QuickEntryPoints, pJniMethodStartSynchronizedFromCode,
                         pReturnProfilingBuffer, sizeof(void*));

    EXPECT_OFFSET_DIFFNP(QuickEntryPoints, pReturnProfilingBuffer,
                         pProfileInvoke, sizeof(void*));

    CHECKED(OFFSETOF_MEMBER(QuickEntryPoints, pProfileInvoke)
