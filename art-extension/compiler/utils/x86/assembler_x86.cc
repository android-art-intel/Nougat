...
X86ConstantAreaReference ConstantArea::AppendInt32(int32_t v) {
  X86ConstantAreaReference result(X86ConstantAreaReference::kBuffer32,
      buffer32_.size() * elem_size_32_);
  buffer32_.push_back(v);
  return result;
}

X86ConstantAreaReference ConstantArea::AddInt32(int32_t v) {
  for (size_t i = 0, e = buffer32_.size(); i < e; i++) {
    if (v == buffer32_[i]) {
      return X86ConstantAreaReference(X86ConstantAreaReference::kBuffer32, i * elem_size_32_);
    }
  }
...
X86ConstantAreaReference ConstantArea::AddInt64(int64_t v) {
  for (size_t i = 0, e = buffer64_.size(); i < e; i++) {
    if (v == buffer64_[i]) {
      return X86ConstantAreaReference(X86ConstantAreaReference::kBuffer64, i * elem_size_64_);
    }
  }

  // Didn't match anything.
  X86ConstantAreaReference result(X86ConstantAreaReference::kBuffer64,
      buffer64_.size() * elem_size_64_);
  buffer64_.push_back(v);
  return result;
}

X86ConstantAreaReference ConstantArea::AddDouble(double v) {
  // Treat the value as a 64-bit integer value.
  return AddInt64(bit_cast<int64_t, double>(v));
}

X86ConstantAreaReference ConstantArea::AddFloat(float v) {
